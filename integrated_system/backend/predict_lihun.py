...
book_data = {
    ...
    'platform': 'Zongheng',
    'status': '连载中',
    'word_count': 8249000,  # 824.9万字
    'total_recommend': 80000,  # 8.0万
    'total_click': 16419000,  # 1641.9万
    'weekly_recommend': 5131,
    'monthly_tickets': 5546,
}

# 需要估算排名（纵横平台，月票5546）
# 纵横月票普遍较低，5546票应该是前10名左右
# 根据纵横历史数据：月票5000+通常是前5-10名
...
print(f"   估算排名: 第{estimated_rank}名（基于月票）")
...
print(f"   实际月票: {book_data['monthly_tickets']}")
...
print(f"\n【优势】")
print(f"   ✓ 月票高（{book_data['monthly_tickets']}票），付费意愿强")
print(f"   ✓ 字数超级充足（{book_data['word_count']/10000:.0f}万字）")
print(f"   ✓ 点击量高（{book_data['total_click']/10000:.0f}万）")
print(f"   ✓ 转化率良好（{result['conversion_rate']*100:.2f}%）")

print(f"\n【题材分析】")
print(f"   现代言情：女性向题材，改编潜力高")
print(f"   适合：电视剧、网剧、短剧改编")

print(f"\n【投资建议】")
if result['ip_grade'] in ['S', 'A']:
    print(f"   ⭐⭐⭐⭐⭐ 强烈推荐")
    print(f"   - IP价值高，商业变现能力强")
    print(f"   - 适合影视改编")
    print(f"   - 女性受众群体庞大")
